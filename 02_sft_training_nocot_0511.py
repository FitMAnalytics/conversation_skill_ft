"""LoRA SFT training for the outbound-sales conversation model — NO-TEACHER-COT VARIANT.

This is a variant of `02_sft_training.py` for use BEFORE trusted teacher
CoT is available. Only the final-channel (polished agent response) is
supervised. The analysis-channel framing tokens are preserved in every
training sequence — `<|channel|>analysis<|message|>` immediately followed
by `<|end|>` — but the span between them is empty and nothing inside the
analysis section contributes to loss.

Intent:
    - Train the final response without contamination from possibly-bad
      teacher CoT.
    - Keep Harmony analysis framing in-distribution so the model is
      familiar with emitting `<|channel|>analysis<|message|>` at inference,
      where the base reasoning prior will still produce content.
    - Observe how the base reasoning behavior survives an SFT that ignores
      the analysis channel.

Known consequence: because the analysis span during training is empty,
the final-channel CE loss never backprops through substantive analysis
tokens. The model never learns to *condition* its final response on
analysis content. Whatever reasoning the base model produces at inference
is therefore decorative, not load-bearing. See `0510_sft_design.md`
Part F for the design discussion.

Input fields read from each JSONL row (the `reasoning` field is
...
contains the SFT target) is never lost.
...
Launch:
    ...
    accelerate launch --config_file <accelerate_config.yaml> 02_sft_training_nocot_0511.py \\
        --config 02_train_config_nocot.yaml
"""
...
DEFAULT_OUTPUT_DIR = "checkpoints_nocot"
...
class TrainConfig:
    ...
    # `reasoning_field` is intentionally absent — this variant doesn't read teacher CoT.
...
def encode_with_spans(tokenizer, row: dict, cfg: "TrainConfig",
    ...
    Analysis-channel framing is emitted but the content span is EMPTY:
    `<|channel|>analysis<|message|><|end|>` with nothing in between. Only
    the final-channel content is tagged for loss.
    """
...
def build_dataset(examples: list[dict], tokenizer, cfg: TrainConfig) -> Dataset:
    ...
    `channel_mask` only ever takes values {0, 2} in this variant — there are
    no analysis-content tokens.
    """
    ...
    for ex in examples:
        ...
        # (containing the SFT target) is never lost.
...
# ============================================================================
# FINAL-ONLY MEAN LOSS TRAINER
# ============================================================================

class FinalOnlyMeanLossTrainer(Trainer):
    """Trainer with per-example final-channel-mean loss.

    For each example: loss = mean(ce[mask == 2]). Batch loss = mean across
    examples. Per-example mean (rather than global token mean) keeps long
    and short finals weighed equally — same invariant the parent script
    enforces via per-channel means.
    """
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        total = torch.stack(per_example_f).mean()

        if model.training:
            self._last_loss_f = float(total.detach().float().item())
        else:
            self._eval_loss_f_sum += float(total.detach().float().item())
            self._eval_count += 1
...
def main() -> None:
    ...
    trainer = FinalOnlyMeanLossTrainer(
        model=model,
        args=training_args,
